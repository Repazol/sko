	...
	init: function(params) {
		...
		this.path = params.path || function() {
			...
			for (var i=-1; ++i<s.length;) {
				if (s[i].getAttribute('src') && -1 != (src = s[i].getAttribute('src')).indexOf('/ajex1.js')) {
					return src.substring(0, src.lastIndexOf('/'));
				}
			}
			alert('Undefined variable "path" in AjexFileManager1.init({path:"/path/to/AjexFileManager1/"});');
			return null;
		}();
